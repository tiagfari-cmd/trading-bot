...
async def update_loop():
    ...
    while True:
        ...
        for mint, data in list(pending_checks.items()):
            ...
            # Updates por marcos de % - envia uma vez por marco, sempre maior que o anterior
            milestones_up = data.get("milestones_up", [])
            if not isinstance(milestones_up, list):
                milestones_up = list(milestones_up)
            data["milestones_up"] = milestones_up
            last_sent_pct = max(milestones_up) if milestones_up else 0
            for milestone in [25, 50, 75, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3500, 4000, 4500, 5000, 6000, 7000, 8000, 9000, 10000, 12000, 15000, 20000]:
                if pct >= milestone and milestone > last_sent_pct:
                    milestones_up.append(milestone)
                    data["milestones_up"] = milestones_up
                    last_sent_pct = milestone
